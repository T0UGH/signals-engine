"""hacker-news-watch lane collector."""
...
def _parse_positive_int(value: object, *, field_name: str) -> int:
    try:
        ...
        raise ValueError(f"hacker-news-watch '{field_name}' must be a positive integer") from exc
    if parsed <= 0:
        raise ValueError(f"hacker-news-watch '{field_name}' must be a positive integer")
    return parsed
...
def _parse_bool(value: object, *, field_name: str) -> bool:
    ...
    if isinstance(value, str):
        ...
        if normalized in {"0", "false", "no", "n", "off"}:
            return False
    raise ValueError(f"hacker-news-watch '{field_name}' must be a boolean")


def _build_signal(ctx: RunContext, story: HackerNewsStory) -> SignalRecord:
    filename = f"hn__{story.story_list_name}__{story.story_id}__story__{ctx.date}.md"
...
def collect_hacker_news_watch(ctx: RunContext) -> RunResult:
    started_at = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S%z")
    lane_config = ctx.config.get("lanes", {}).get("hacker-news-watch", {})
    if not isinstance(lane_config, dict):
        ctx.errors.append("hacker-news-watch config must be a mapping")
        return _finalize(ctx, started_at, [], 0)

    try:
        story_list = validate_story_list(lane_config.get("story_list", "top"))
        max_stories = _parse_positive_int(lane_config.get("max_stories", 10), field_name="max_stories")
    ...
    debug_log(
        (
            "[hacker-news-watch] START "
            f"story_list={story_list} max_stories={max_stories} "
    ...
    try:
        stories = fetch_hackernews_stories(
    ...
    for story in stories:
        try:
            ...
            debug_log(
                (
                    "[hacker-news-watch] + "
                    f"{story.story_id} {story.story_list_name} score={story.score} comments={story.descendants}"
                ),
                log_file=ctx.debug_log_path,
            )
    ...
    return _finalize(ctx, started_at, records, len(stories))


def _finalize(ctx: RunContext, started_at: str, records: list[SignalRecord], stories_checked: int) -> RunResult:
    ...
    debug_log(
        f"[hacker-news-watch] END signals={len(records)} stories={stories_checked}",
        log_file=ctx.debug_log_path,
    )
    return result
...
register_lane("hacker-news-watch", collect_hacker_news_watch)
