...
class TestCliEntrypoint(unittest.TestCase):
    ...
    def test_cli_module_invocation_runs_main(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "signals_engine.cli", "--help"],
            capture_output=True,
            text=True,
        ...
        self.assertIn("signals-engine", result.stdout.lower())

    def test_collect_command_is_executable(self):
        """`python -m signals_engine.cli collect --help` should succeed."""
        result = subprocess.run(
            [sys.executable, "-m", "signals_engine.cli", "collect", "--help"],
            capture_output=True,
            text=True,
            cwd=str(Path(__file__).parent.parent),
            env={**subprocess.os.environ, "PYTHONPATH": "src"},
    ...
    def test_diagnose_command_is_executable(self):
        """`python -m signals_engine.cli diagnose --help` should succeed."""
        result = subprocess.run(
            [sys.executable, "-m", "signals_engine.cli", "diagnose", "--help"],
            capture_output=True,
            text=True,
            cwd=str(Path(__file__).parent.parent),
            env={**subprocess.os.environ, "PYTHONPATH": "src"},
